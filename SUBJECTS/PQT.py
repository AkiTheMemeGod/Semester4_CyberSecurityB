...
def app():
    center_title(60, "#0C2637", "21MAB204T<br>Probability and Queueing Theory")
    # st.title(":red[21CSC205P] - Database Management Systems")
    data = Database()

    st.header("Notes 👇")
    option = st.selectbox("Select the pdf you want to fetch : ",
                          options=data.doc_list("pqt"), label_visibility="hidden",
                          placeholder="Choose the document from here",
                          index=0)

    name, bin_data = data.get_data("pqt", option)
    st.markdown("###")
    st.markdown("###")
    st.markdown("###")
